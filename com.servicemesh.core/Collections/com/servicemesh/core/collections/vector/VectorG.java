...
/**
 * VectorG provides a resizable array of Generic type with an assortment of capabilities such as the ability to fill sections,
...
public class VectorG<T>
{
     ...
     *            the value to initialize new allocated entries to.
     */
    public void put(int index, T[] values, int offset, int length, T missingValue)
    {
        if ((index + length) > m_used)
        {
    ...
    /**
     * Grow the values array. All values are copied to the expanded array and new entries are initialized to a specified missing
     * value.
     *
     * @param size
     *            the new size of the values array.
    ...
    /**
     * Returns an array containing all of the elements in this vector in the same order in which they appear in the vector; the
     * runtime type of the returned array is that of the specified array. If all of the values fit in the specified array, thet
     * are returned therein. Otherwise, a new array is allocated with the runtime type of the specified array and the size of this
     * vector.
     * 
     * @param a
    ...
    public <U> U[] toArray(U[] a)
    {
        if (a.length < m_used)
        {
            ...
            a = (U[]) java.lang.reflect.Array.newInstance(a.getClass().getComponentType(), m_used);
        }
        System.arraycopy(m_values, 0, a, 0, m_used);
        return a;
    }
