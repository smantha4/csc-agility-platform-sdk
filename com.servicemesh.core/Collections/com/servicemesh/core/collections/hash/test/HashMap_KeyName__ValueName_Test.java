...
public class HashMap_KeyName__ValueName_Test
{
    ...
    public void testInitialCondition()
    {
        ...
        assertEquals("Map values should initially be empty", 0, m_map.getValues().length);
        assertEquals("Map keys should initially be empty", 0, m_map.getKeys().length);
        Itemizer itm = m_map.itemizer();
        assertFalse("Itemizer for empty map should not show more elements", itm.hasMore());
    }
    ...
    public void testSimplePut()
    {
        ...
        assertTrue("Map should contain new key", m_map.containsKey(Types.get_KeyName_For(100)));
        assertKeyEquals("Map keys should contain new value", Types.get_KeyName_For(100), m_map.getKeys()[0]);
        assertValueEquals("Map values should contain new value", Types.get_ValueName_For(200), m_map.getValues()[0]);
    }
    ...
    private void doTestItemizer(int count)
    {
        ...
        for (int i = 1; i <= count; i++)
        {
            ...
            expected.put(Types.get_KeyName_For(i), Types.get_ValueName_For(i * 2));
        }
    ...
    protected void doTestIteratorContents(Itemizer itm, int count)
    {
        for (int i = 1; i < count; i++)
        {
            ...
            assertTrue(Comparators._ValueName_Asc.equals(m_map.get(m_map.getEntryKey(itm.entry())),
