...
public class WorkReactor implements Reactor, Runnable
{
    ...
    /**
     * The number of passes through the reactor loop. Allowed to wrap. Useful for debugging.
     */
    protected long m_cycleCount;

    /**
     * Returns a default WorkReactor instance. Every call to this method returns the same WorkReactor instance.
     *
    ...
    /**
     * Constructs a WorkReactor instance. The constructor does not start the thread. It is necessary to call start() after the
     * WorkReactor has been constructed.
     *
     * @param name
     *            the name to assign to the WorkReactor thread.
     */
    ...
    /**
     * Constructs a WorkReactor instance. The constructor does not start the thread. It is necessary to call start() after the
     * WorkReactor has been constructed. This constructor will create a Thread with the name "WorkReactor_N" where N is the number
     * of WorkReactors created so far.
     */
    ...
    public void workSubmit(Work work)
    {
        if (!work.isPending())
        {
            throw new IllegalStateException("Attempt to submit a Work that is already busy.");
        }
    ...
    /**
     * Determines if there is work pending. We check to see if there is any work on the queue at all.
     *
    ...
    protected void doWork()
    {
        ...
        for (Work work = m_activeWorks.getRight(); work != m_activeWorks; work = m_activeWorks.getRight())
        {
