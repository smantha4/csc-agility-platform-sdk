...
public class ServiceAdapterTest {
	...
	public void testUnAuthGetProxy() throws Exception {
		...
		Assert.assertTrue("The first Proxy was not given hostname correctly", returnProxyList.get(0).toString().equals("http//dummyHost->http//dummyHost2"));
		Assert.assertTrue("The second Proxy was not given hostname correctly", returnProxyList.get(1).toString().equals("http//dummyHost2"));
		Assert.assertTrue("The test Proxy was not given Proxy Type correctly", returnProxyList.get(0).getType().equals(com.servicemesh.io.proxy.ProxyType.HTTP_PROXY));
	}
	...
	public void testAuthGetProxy() throws Exception {
		...
		Assert.assertTrue("The test Proxy was not given hostname correctly", returnProxyList.get(0).getHostname().equals("dummyHost"));
		Assert.assertTrue("The test Proxy was not given Proxy Type correctly", returnProxyList.get(0).getType().equals(com.servicemesh.io.proxy.ProxyType.HTTPS_PROXY));
	}
	...
	public void testAssetCRUD() throws Exception
	{
		...
		when(mockAsync.promise(any(Request.class)))
			.thenAnswer(new Answer< Promise<Response> >() {
					@Override
		...
		when(mockAsync.sequence(any(ApiRequest.class)))
			.thenAnswer(new Answer< Promise< List<ApiResponse> > >() {
					@Override
			...
			if (succeed) {
				...
				if (obj instanceof ServiceProvider) {
					...
					for (int i = 0 ; i < providers.size() ; i++) {
						Assert.assertEquals(providers.get(i).getId(),
											sps.get(i).getId());
					}
				}
			}
