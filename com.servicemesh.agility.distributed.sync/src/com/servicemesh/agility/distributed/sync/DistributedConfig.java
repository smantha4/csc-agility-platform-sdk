...
public class DistributedConfig implements Watcher {
	...
	/**
	 * Attempts to find the thread that Zookeeper spawned to send heartbeat 
	 * messages to the ZooKeeper server and raise its priority to MAX_PRIORITY.
	 * 
	 *  Warning - Fragile: dependent on ZooKeeper internals
	 */
	private static void bumpZkHeartbeatPriority() {
	    ...
	    for (int i=0; i<numThreads; i++) {
	        ...
	        if (thread.getName().contains("SendThread")) {  // see org.apache.zookeeper.ClientCnxn$SendThread
     ...
     * @param path The path to the ZooKeeper node that will hold data
     * @param mode The ZooKeeper persistence mode for the node
     */
     ...
     * @param path The path to the ZooKeeper node that will hold data
     * @param mode The ZooKeeper persistence mode for the node
     * @param watcher An implementation of the ZooKeeper Watcher interface to monitor data changes
     */
     ...
     * @param path The path to an existing ZooKeeper node
     * @param watcher An implementation of the ZooKeeper Watcher interface to monitor data changes
     */
