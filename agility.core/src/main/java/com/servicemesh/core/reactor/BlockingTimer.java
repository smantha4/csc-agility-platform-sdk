...
/**
 * Timer class that, when firing, waits until it is externally signaled to proceed, and also allows other threads to know when the
 * timerFire method has started. This will cause the reactor that is firing it to block until the signal is received. Useful for
 * guaranteeing checkpoints and rendezvous during tests and perhaps dealing with some odd race conditions.
 */
