...
/**
 * This is an abstract base class for array oriented heap data structures (sets, maps, and tables) with Generic keys. This
 * provides the common machinery used by subclasses to implement heap data structures with Generic keys and values of arbitrary
 ...
 */
public abstract class AbstractHeapG<K> extends AbstractHeap
{
    /**
    ...
    public K[] getKeys(K[] dst)
    {
        ...
        {
            dst = (K[]) Array.newInstance(dst.getClass().getComponentType(), m_size);
        }
        for (int i = 0; i < m_size; i++)
        {
