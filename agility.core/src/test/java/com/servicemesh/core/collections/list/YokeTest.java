...
public class YokeTest
{
    ...
    protected static class IntYoke extends Yoke<IntYoke>
    {
         ...
         *
         * @param value
         *            the value to place in this Yoke.
         */
    ...
    protected static Orientation s_leftOrientation = new Orientation() {
        ...
        public void insertBefore(IntYoke a, IntYoke b)
        {
            a.insertLeft(b);
        }

        public void insertAfter(IntYoke a, IntYoke b)
        {
            a.insertRight(b);
        }

        public IntYoke getBefore(IntYoke j)
        {
            return j.getLeft();
        }

        public IntYoke getAfter(IntYoke j)
        {
            return j.getRight();
        }
    };
    ...
    {
        assertEquals("Head A doesn't point right to itself", m_headA, m_headA.getRight());
        assertEquals("Head A doesn't point left to itself", m_headA, m_headA.getLeft());

        assertEquals("Head B doesn't point right to itself", m_headB, m_headB.getRight());
        assertEquals("Head B doesn't point left to itself", m_headB, m_headB.getLeft());
    }
    ...
    protected void doInserts(Orientation o)
    {
        ...
        assertTrue("Before and after lengths don't match",
                aAfterLength == aBeforeLength && aAfterLength == bAfterLength && aAfterLength == bBeforeLength);
    ...
    public void doListInserts(Orientation o)
    {
        ...
        assertEquals(o.direction() + ": Head A is not before start", m_headA, o.getBefore(seqStartA));
        assertEquals(o.direction() + ": Head A is not after end", m_headA, o.getAfter(seqEndA));
        assertEquals(o.direction() + ": Start A is not after head", seqStartA, o.getAfter(m_headA));
        assertEquals(o.direction() + ": End A is not before head", seqEndA, o.getBefore(m_headA));

        verifyLengthAndSequence(o, m_headA, SIZE_A + 1, 1, SIZE_A, SIZE_A, SIZE_A);

        // Verify that the insert is reversible
        o.insertBefore(m_headA, seqStartA);
        testInitialConditions();
        assertEquals(o.direction() + ": Sequence A end is not before start", seqEndA, o.getBefore(seqStartA));
        assertEquals(o.direction() + ": Sequence A start is not after end", seqStartA, o.getAfter(seqEndA));
        ...
        verifyLengthAndSequence(o, m_headB, SIZE_B + 1, SIZE_A + 1, SIZE_B, SIZE_A + SIZE_B, SIZE_B);

        // Let's splice the two lists together
        o.insertBefore(m_headA, m_headB);

        // Verify that head A was inserted in the right place
        assertEquals(o.direction() + ": Start A is not after head A", seqStartA, o.getAfter(m_headA));
        assertEquals(o.direction() + ": End B is not before head A", seqEndB, o.getBefore(m_headA));
        assertEquals(o.direction() + ": Head A is not after end B", m_headA, o.getAfter(seqEndB));
        assertEquals(o.direction() + ": Head A is not before start A", m_headA, o.getBefore(seqStartA));

        // Verify that head B was inserted in the right place
        assertEquals(o.direction() + ": Start B is not after head B", seqStartB, o.getAfter(m_headB));
        assertEquals(o.direction() + ": End A is not before head B", seqEndA, o.getBefore(m_headB));
        assertEquals(o.direction() + ": Head B is not after end A", m_headB, o.getAfter(seqEndA));
        assertEquals(o.direction() + ": Head B is not before start B", m_headB, o.getBefore(seqStartB));

        // Verify the combined lists
        verifyLengthAndSequence(o, m_headA, SIZE_A + SIZE_B + 2, 1, SIZE_A, SIZE_A + SIZE_B, SIZE_B);
        verifyLengthAndSequence(o, m_headB, SIZE_A + SIZE_B + 2, SIZE_A + 1, SIZE_B, SIZE_A, SIZE_A);
    ...
    protected void verifyLengthAndSequence(Orientation o, IntYoke head, int length, int startAfter, int countAfter, int endBefore,
            int countBefore)
    {
        // Verify lengths
        assertEquals(o.direction() + ": List has wrong length after", length, getAfterLength(o, head));
        assertEquals(o.direction() + ": List has wrong length before", length, getBeforeLength(o, head));
    ...
    protected boolean isAfterInOrder(Orientation o, IntYoke j, int start, int count)
    {
    ...
    protected boolean isBeforeInOrder(Orientation o, IntYoke j, int end, int count)
    {
        for (int i = 0; i < count; i++)
        {
            if (j.getValue() != end--)
